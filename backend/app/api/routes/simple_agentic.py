...
router = APIRouter(prefix="/simple-agentic", tags=["simple-agentic"])
...
def test_simple_agentic():
    ...
    try:
        ...
        return {
            "status": "success", 
            "message": "Simple agentic system is working", 
            "ai_response": response[:200] + "..." if len(response) > 200 else response
        }
    except Exception as e:
        import traceback
        print(f"Error in simple agentic test: {str(e)}")
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/run")
def run_simple_agentic_query(user_id: str, session_id: str, query: dict):
    try:
        ...
        import traceback
        print(f"Error in simple agentic query: {str(e)}")
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=str(e))
